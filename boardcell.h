...
class BoardCell {
    ...
    BoardCell() {} // default contstructor (do nothing)
      ...
      virtual void attemptMoveTo(size_t& newR, size_t& newC, size_t hRow, size_t hCol) {
            ...
            newC = getCol(); // modify/remove this line
    ...
    public:
      Hero(size_t r, size_t c) {
        ...
        virtual void attemptMoveTo(size_t& newR, size_t& newC, size_t hRow, size_t hCol) {
            ...
            //this checks nextMove to see which way the hero has to go
            // 'q' = up and left, 'w' = up, 'e' = up and right
            // 'a' = left, 's' = stay, 'd' = right
            // 'z' = down and left, 'x' = down, 'c' = down and right
            // Interpret ANY other input as 's' = stay
...
class Monster: public BoardCell {
    ...
    Monster(size_t r, size_t c) {
      ...
      virtual void attemptMoveTo(size_t& newR, size_t& newC, size_t hRow, size_t hCol) {
            ...
            //this checks which way the monster has to go to travel towards to the hero's row
            if (hRow > getRow()) {
                vertDir = 1;
            } else if (hRow < getRow()) {
                vertDir = -1;
            ...
            ////this checks which way the monster has to go to travel towards to the hero's column
            if (hCol > getCol()) {
                horDir = 1;
            } else if (hCol < getCol()) {
                horDir = -1;
            ...
            // Super Monsters move 2 places forward
            if (getPower() == 2) {
                newR += vertDir;
                newC += horDir;
            }
...
class Bat: public BoardCell {
    ...
    Bat(size_t r, size_t c) {
      ...
      virtual void attemptMoveTo(size_t& newR, size_t& newC, size_t hRow, size_t hCol) {
            ...
            //bats cannot change rows but always go to the hero's column
            newR = getRow(); // modify/remove this line
