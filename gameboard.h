    ...
    void display( ) {
      ...
      //      int HeroCol;
        ...
        void setHeroPosition(size_t row, size_t col) {
            ...
            //---------------------------------
            HeroRow = static_cast<size_t>(row);
            HeroCol = static_cast<size_t>(col);
      ...
      //      int HeroCol;
        ...
        void findHero() {
            ...
            for(size_t r = 0; r < board.numrows(); r++){
                for(size_t c = 0; c < board.numcols(r); c++){
                    if(board(r,c)->display() == 'H'){//checks if the positon is the hero
                        HeroRow = static_cast<int>(r);
                        HeroCol = static_cast<int>(c);
                        return;//ensd the function call, HeroRow and Col set to the postion of the hero
                    }
                }
            }
            //if the hero isnt found
            HeroRow = -1;
            HeroCol = -1;
        }
        ...
        bool makeMoves(char HeroNextMove) {
            ...
            bool rowOrCol;//true means invalid row, false invalid col
            bool monCapHero = false;
            try {
                // hero attempts to move out-of-bounds in rows

                if (newR < 0 || newR >= numRows) {//invalid row
                    if(newR < 0) {
                        newR = 0;
                    }
                    else if (newR >= numRows) {
                        newR = numRows - 1;
                    }
                    rowOrCol = true;
                    throw runtime_error("Hero trying to move out-of-bounds with an invalid row");
                }
                else if(newC < 0 || newC >= numCols){//invalud col
                    if (newC < 0) {
                        newC = 0;
                    }
                    else if (newC >= numCols) {
                        newC = numCols - 1;
                    }
                    rowOrCol = false;
                ...
                char moveToCell = board(newR, newC)->display();

                if(moveToCell == ' '){//hero is able to move there
                ...
                else if(moveToCell == '#'){//hero fell into the abyss and died
                    board(HeroRow, HeroCol) = new Nothing(HeroRow, HeroCol);
                    HeroRow = -1;
                    HeroCol = -1;
                    wonGame = false;
                }
                else if(moveToCell == '*'){// hero won
                    board(HeroRow, HeroCol) = new Nothing(HeroRow, HeroCol);
                    HeroRow = -1;
                    HeroCol = -1;
                    wonGame = true;
                }
                else if(moveToCell == 'm' || moveToCell == 'M' || moveToCell == '~'){//hero hit and monster and died
                    board(HeroRow, HeroCol) = new Nothing(HeroRow, HeroCol);
                    HeroRow = -1;
                    HeroCol = -1;
                    wonGame = false;
                ...
                else if (moveToCell == '+') {//hero attemps to move into wall
                    if(!(HeroNextMove == 'w' || HeroNextMove == 'x' || HeroNextMove == 's' || HeroNextMove == 'a' || HeroNextMove == 'd')){
                        if(board(newR, HeroCol)->display() != '+'){//the moves for only diagonal
                            if(board(newR, HeroCol)->display() == ' '){//checks if the space is not a monster
                        ...
                        else{//ignores vert component of movement
                ...
                if(HeroNextMove == 'q' || HeroNextMove == 'e' || HeroNextMove == 'z' || HeroNextMove == 'c'){
                    ...
                    if(he && board(newR,newC)->display() == ' '){
                        board(newR, newC) = board(HeroRow, HeroCol);
                        board(HeroRow, HeroCol) = new Nothing(HeroRow, HeroCol);
                        HeroRow = static_cast<int>(newR);
                        HeroCol = static_cast<int>(newC);      
                    }
                    else if(he && board(newR,newC)->display() == '+'){}
                    else if(he && board(newR,newC)->display() == '*'){
                        board(HeroRow, HeroCol) = new Nothing(HeroRow, HeroCol);
                        HeroRow = -1;
                        HeroCol = -1;
            ...
            for(size_t r = 0; r < numRows; r++){
                for (size_t c = 0; c < numCols; ++c) {
                    // Checks if it is a unmoved Baddie
                    if (board(r, c)->isBaddie() && !board(r, c)->getMoved()) {
                        // Get the attempted move for the Baddie
                        size_t newRow, newCol;
                        board(r, c)->attemptMoveTo(newRow, newCol, HeroRow, HeroCol);

                        try {
                            // Baddie attempts to move out-of-bounds in rows
                            if (newRow < 0 || newRow >= numRows) {
                                newRow = (newRow < 0) ? 0 : (newRow >= numRows) ? numRows - 1 : newRow;
                            ...
                            if (newCol < 0 || newCol >= numCols) {
                                newCol = (newCol < 0) ? 0 : (newCol >= numCols) ? numCols - 1 : newCol;
                                throw runtime_error("Baddie trying to move out-of-bounds with an invalid column");
                            }

                            // the type of cell the Baddie is attempting to move to
                            char target = board(newRow, newCol)->display();

                            // different types of cells
                            if (target == ' ') {//the space is free
                                board(newRow, newCol) = board(r, c);
                            ...
                            } else if (target == '#') {//baddie dead
                                board(r, c) = new Nothing(r, c);
                                board(r, c)->setMoved(true);
                            } else if (target == 'H') {//hero dead
                                newRow = HeroRow;
                                newCol = HeroCol;
                                board(HeroRow, HeroCol) = board(r,c);
                                board(r, c) = new Nothing(r, c);
                                board(newRow, newCol)->setRow(newRow);
                                board(newRow, newCol)->setCol(newCol);
                                board(newRow, newCol)->setMoved(true);
                                monCapHero = true;
                            } else if (target == '+' || target == '*') {//no move as the baddei hit a wall or the ladder
                                board(r, c)->setMoved(true);
                            } else if (target == 'm' || target == 'M' || target == '~') {//baddies hit each other
                                board(r, c)->setMoved(true);
                            }
                        } catch (runtime_error &excpt) {
                            cout << excpt.what() << endl;
